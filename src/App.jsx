    ...
    <div style={styles.page}>
      ...
      <form onSubmit={onSubmit} style={styles.card}>
        <div style={styles.grid}>
          ...
          <Field label="内職コード（必須 / QR可）">
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                style={styles.input}
              />
              <button type="button" onClick={() => setQrOpen(true)} style={styles.btn}>
                QR
              </button>
              <button
                type="button"
          ...
          <Field label="商品（必須）">
            <select value={product} onChange={(e) => setProduct(e.target.value)} style={styles.select} disabled={!!jobCode}>
              <option value="">選択して</option>
          ...
          <Field label="工程（必須）">
            <select value={process} onChange={(e) => setProcess(e.target.value)} style={styles.select} disabled={!!jobCode}>
              <option value="">選択して</option>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 14 }}>
          <button type="button" onClick={resetForNext} style={styles.btnGhost}>次の入力へ（クリア）</button>
        ...
        <div style={styles.modalBg} onClick={() => setQrOpen(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <b>QRで内職コード読み取り</b>
              <button onClick={() => setQrOpen(false)} style={styles.btnGhost}>閉じる</button>
            </div>
...
const styles = {
  ...
  card: { background: "#fff", border: "1px solid #eee", borderRadius: 16, padding: 16, boxShadow: "0 6px 24px rgba(0,0,0,0.06)" },
